...
class App(ctk.CTk):
    ...
    def _make_scroll_frame(self, parent):
        """Create a scrollable frame using tkinter Canvas"""
        ...
        # Mousewheel: only when mouse is over this canvas
        def _enter(e): canvas.bind_all("<MouseWheel>", lambda ev: canvas.yview_scroll(int(-1*(ev.delta/120)), "units"))
    ...
    def _build_game_page(self, gname):
        ...
        btn_col = ctk.CTkFrame(hi, fg_color="transparent")
        btn_col.pack(side="right")
        make_btn(
            btn_col, "▶  ЗАПУСТИТЬ " + gname.upper(),
            lambda gn=gname: self._launch_game(gn),
            size=12, width=200,
            color="#006adf", hover="#00aaff",
        ).pack(pady=(0,4))
        make_btn(
            btn_col, "⚡  ОПТИМИЗИРОВАТЬ ВСЁ",
        ...
        for tname in tab_names:
            ...
            _inner = self._make_scroll_frame(frame)
            inner = ctk.CTkFrame(_inner, fg_color="transparent")
    ...
    def _kill_game_parasites(self, gname):
        ...
        def run():
            ...
            for cmd in extra:
                run_cmd(cmd); count += 1
            # Deep file edit - write directly to game cfg
            if log: self._log_write(log, "📁 Записываю в файлы игры...")
            results = self._deep_edit_game(gname)
            for status, path in results:
                short = path[-45:] if len(path) > 45 else path
                if log: self._log_write(log, "  " + status + " " + short)
            if log:
                self._log_write(log, "  ✓ " + str(count) + " паразитов через реестр")
    ...
    def _launch_game(self, gname):
        """Find game exe and launch it directly"""
        import os.path
        paths_data = GAME_PATHS.get(gname, {})
        exe_paths = paths_data.get("exe_paths", [])
        steam_id = paths_data.get("steam_id")
        epic_id = paths_data.get("epic_id")

        # Expand env vars and check each path
        ...
        # Fallback: Steam launch
        if steam_id:
            subprocess.Popen(["cmd", "/c", "start", f"steam://rungameid/{steam_id}"],
                             shell=False)
            return

        # Fallback: Epic launcher
        if epic_id:
            subprocess.Popen(["cmd", "/c", "start",
                             f"com.epicgames.launcher://apps/{epic_id}?action=launch"],
                             shell=False)
            return

        # Show not found popup
        win = ctk.CTkToplevel(self)
        win.title("Игра не найдена")
        win.geometry("420x200")
        win.configure(fg_color=BG)
        ctk.CTkLabel(win, text="⚠️ Не удалось найти " + gname,
                     font=ctk.CTkFont(size=14, weight="bold"),
                     text_color=GOLD).pack(pady=(20,8))
        ctk.CTkLabel(win, text="Убедись что игра установлена.\nЕсли путь нестандартный — запусти через Steam/Epic.",
                     font=ctk.CTkFont(size=12), text_color=TEXT,
                     justify="center").pack(pady=4)
        make_btn(win, "Закрыть", win.destroy, width=120,
                 color="#1a3050", hover="#2a4060").pack(pady=16)

    def _deep_edit_game(self, gname):
        """Write directly to game cfg files to apply parasite kills"""
        import os.path
        paths_data = GAME_PATHS.get(gname, {})
        cfg_paths = paths_data.get("cfg_paths", [])
        cfg_parasites = paths_data.get("cfg_parasites", [])
        results = []

        for cfg_template in cfg_paths:
            cfg_path = os.path.expandvars(cfg_template)
            ...
            try:
                ...
                if os.path.exists(cfg_path):
                    ...
                    existing = ""
                # Append parasite kills
                new_lines = []
                for line in cfg_parasites:
                    if line not in existing:
                        new_lines.append(line)
                if new_lines:
                    with open(cfg_path, "a", encoding="utf-8") as f:
                        f.write("\n// Game Optimizer - parasite kill\n")
                        f.write("\n".join(new_lines) + "\n")
